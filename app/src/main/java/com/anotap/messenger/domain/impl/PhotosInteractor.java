...
import com.anotap.messenger.domain.IPhotosInteractor;
...
import com.anotap.messenger.model.AccessIdPair;
...
public class PhotosInteractor implements IPhotosInteractor {
    ...
    public Single<List<Photo>> getPhotosByIds(int accountId, Collection<AccessIdPair> ids) {
        List<com.anotap.messenger.api.model.AccessIdPair> dtoPairs = new ArrayList<>(ids.size());

        for(AccessIdPair pair : ids){
            dtoPairs.add(new com.anotap.messenger.api.model.AccessIdPair(pair.getId(),
